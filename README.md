...
- E-01 已部署 `aneb-server/0.5.1`：Token SSE（单任务上传硬上限 128MiB）、实时交互 WebSocket、连接级受控中断、HTTP/3 和与 H3 共用 UDP/8443 的带序号应用探针；`s3_multimodal@0.3.0` 新增两段 12MiB `download_burst`。权威能力与部署纪律见 `docs/TEST_SERVER_CAPABILITIES.md`。
...
- 最终质量门：431 JVM tests、0 failures（64 suites，按 XML suite header 复算）；Android Lint 0 errors；行为模型 14 tests PASS；Go tests PASS；0.4.6 Debug APK 已生成。
