...
## 当前状态（2026-07-19，已验收 0.5.10 / 历史负向 0.5.11 / 本地候选 0.5.12）
...
- M0-EC1 把 `token_multimodal_quick@1.2.1` 接到 P1、P2 0.8.0 与 P3 0.3.1 的同一执行合同：APK 在首个 echo/Token/download 业务请求前核对本地 manifest、精确 Profile 摘要和节点能力回执，不兼容时 fail closed 并抑制评分；Quick 运行计划也真实执行一个模型派生的 1MiB 返回附件，避免“声明 download、实际不下载”。0.5.11 云端 Debug 工件只完成了现场 0.7 语境下的客户端负向验证；新 0.5.12/code 44 候选进一步为同一 Token run 的控制与业务请求绑定规范 UUID，并用固定审计角色区分 reachability 与 capability。服务端证据只有在唯一 start/end 双 barrier、同一进程实例、连续序号、能力门先于业务且窗口内无 drop/未归因业务时才可判定；其含义仅为 request-entry coverage，必须与同 run 客户端冻结结果结合，不能单独证明请求成功。最终 44 文件暂存快照的本地全仓门禁与凭据扫描已通过，但尚无新 commit 或 0.5.12 云端 APK；E-01 仍不得写成已部署 0.8.0。
