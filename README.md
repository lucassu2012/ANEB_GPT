...
- E-01 已部署 `aneb-server/0.5.0`：Token SSE（单任务上传硬上限 128MiB）、实时交互 WebSocket、HTTP/3 和与 H3 共用 UDP/8443 的带序号应用探针。
- Token、AI 实时和网络综合均已在 P40 Pro 完成 Quick 端到端验收；Token Stress 还完成了 100MiB 上行 + 100MiB 下行真机闭环。AI 实时结果现冻结 Profile 声明的全部 21 项指标，并在语音负载期间并发刷新 loaded RTT；Room 已到 v15。
- 最终质量门：442 JVM tests、0 failures；Android Lint 0 errors；行为模型 13 tests PASS；Go tests PASS；0.4.1 Debug APK 已生成。
- 当前续开发状态、真机 run 与维护入口见 `docs/CLOUD_CONTINUATION_2026-07-16.md`；版本化测量裁定见 `docs/DECISION_LOG.md` D-36～D-44。
