...
- 最终质量门：501 JVM tests、0 failures；Android Lint 0 errors（10 项依赖/SDK/API 版本提示）；行为模型 14 tests PASS；Go 服务端/网关 tests PASS；Debug/Release API 入口边界 PASS。0.5.1 Debug APK 已生成，SHA-256 为 `099C9180D7EFE5FDEAF1F4A96FA9D74E5ED0B59ABE5141676128CEE6A170426A`。
- P40 Pro App 0.5.1 真机 run `019f709d-33bf-7dbf-a732-35e28a71b447` 完成 AI 实时 Quick：1/1 会话、3 轮、Room 写入 `ok=true`、98.6/A、`LOW/INCONCLUSIVE`、终态 completed；Debug API 入口缺参安全拒绝也通过。测试后已回桌面并强停 Codex 包，两套 ANEB 均无 PID/服务；无效全黑截图未计入 UI 证据，见 `docs/P40_APP_0.5.1_VALIDATION_2026-07-17.md`。
- “3 个子项目 + 1 个横切机制”的事实进度与 M0～M4 验收差距见 `docs/PLAN_ALIGNMENT_2026-07-17.md`；当前续开发状态、真机 run 与维护入口见 `docs/CLOUD_CONTINUATION_2026-07-16.md`；版本化测量裁定见 `docs/DECISION_LOG.md` D-36～D-54。
