...
- `spec/` 逻辑根现为 catalog 1.10.0：索引 10 个 Schema、2 个 Profile 家族、16 个 Profile、7 个哈希绑定运行包，以及 D-110 的唯一重复性资格政策和资格报告合同。结果合同分为兼容 v1、严格 v2 与共享核心；Room v19 为 Token、AI 实时和网络综合新 run 同事务保存类型化行、统一结果信封、1Hz 无线样本与环境事件，三类结果页均可保存/分享原样 JSONL。D-110 分析器核心与 CLI 已能分开评估 Q1/Q2、三族重复性、无线 cadence 和 Profile 质量门，但 qualification Profile/runtime 与真实 Q1/Q2 采集尚未完成。R01 通过证据引用表达时间序列，权限拒绝/不可用/未采集保持显式状态，共享信封不含位置。
