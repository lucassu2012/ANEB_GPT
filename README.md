...
- M0 已建立 `spec/` 逻辑根和机器目录：索引 3 个 Schema、2 个 Profile 家族、16 个 Profile 与 6 个哈希绑定运行包。统一 `aneb-result-v1` 已冻结并通过 Draft 2020-12 正反例校验；Room v19 首先为 Token 新 run 同事务保存类型化行与统一结果信封，未采集无线指标明确写 `not_collected`。AI 实时/网络综合信封、正式三引擎 RadioCollector 和用户可见 JSONL 分享仍待接线，不计为完成。
...
- 0.5.2 最终质量门：509 JVM tests、0 failures；Android Lint 0 errors（10 项依赖/SDK/API 版本提示）；行为模型 21 tests PASS；Go 服务端/网关 tests PASS；Debug/Release API 入口边界 PASS。Debug APK 包名 `com.aneb.probe.codex`、versionCode 34，SHA-256 为 `91261db156129cc81c40ee91e10bf8a151d95988e435e66363ec7628270a45bd`。
...
- “3 个子项目 + 1 个横切机制”的事实进度与 M0～M4 验收差距见 `docs/PLAN_ALIGNMENT_2026-07-17.md`；当前续开发状态、真机 run 与维护入口见 `docs/CLOUD_CONTINUATION_2026-07-16.md`；版本化测量裁定见 `docs/DECISION_LOG.md` D-36～D-55。
