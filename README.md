...
- 云端 CI 不再只“编译但不交付”：`main` 与 `codex/**` 分支通过 Profile/结果合同、18 项脚本测试、服务器、网关、行为模型、Android、Release 边界后，才生成带 APK 身份、签名、SHA-256、中文安装说明的 Debug 候选；非 PR 构建追加 GitHub provenance attestation，并保留可下载工件 30 天。Debug 候选不冒充正式签名 Release。
