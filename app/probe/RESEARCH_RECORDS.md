...
手工表单已接入（见下节）；尚未完成真实媒体核验、设备/UI实测。未保存的原文/分析导入预览在Activity重建后需重新选择文件；已确认保存的原文和分析副本不受影响。
手机验收由2a执行；Android全量质量门按PMO构建队列安排，不操作设备/签名/发布。

## 新建 / 继续手工研究记录

在App研究记录中进入手工表单，主动选择SAMPLE或OBSERVED，填写App/版本/模式及固定动作的三个尝试槽位。是否发送、执行结局、可见完成、指令满足独立填写；已发送但缺T0仍是已发送，不把失败或未知改成未执行。OBSERVED必须有已有现场事实声明和每槽来源/说明，空计划不能自动保存成实测。

仅录入已有的时间/帧区间与依据；时间用毫秒，帧不自动换算。未知留空、真实0保留，不生成TTFR/TTFC/Completion/stall/RPI。校验失败可继续修正，本机草稿不进入正式记录/尝试计数。正式记录保存失败不会清草稿；成功后进入现有详情、重开及原JSON导出。

草稿按Android本地机制暂存，不是云同步；清数据/卸载会删除。实际键盘操作、强杀恢复和底层写入故障未做真机验证。损坏草稿需明确丢弃，不自动覆盖。测试使用的OBSERVED仍为TEST ONLY虚构输入。
