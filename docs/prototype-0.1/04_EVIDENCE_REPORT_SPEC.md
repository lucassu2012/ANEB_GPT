...
```json
{
  ...
  "android_app": {
    "version_name": "<string>",
    "version_code": 1,
    "apk_sha256": "sha256:<hex>"
  },
  "server": {
    "version": "<string>",
    "binary_sha256": "sha256:<hex>",
...
- `event_type`;
- `client_monotonic_ns` when generated by Android;
...
For `content_event`, `details` includes `seq`, `planned_offset_ms`, `schedule_hash` and parsed identity fields. It must not include third-party content, credentials or arbitrary payload bytes.
...
- Condition and schedule hashes match server receipts.
- `task_success=true` implies `run_status=complete`, exact 120 events and valid terminal receipt.
- `score_eligible=true` implies task success and compatible contracts.
