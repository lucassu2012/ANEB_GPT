...
G0 passed through PR #23 and merge commit `a3e9e066db39a4869db8a40bd2b59cd43fc456b7`; issue #14 is closed. G1 passed through PR #27 and merge commit `9b968378ab7fbc8ae6c69838f6d165d8377f07c2`; issue #15 is closed. Issue #16 has merged the exact Prototype projector through PR #29, the real server `done` SSE fixture plus strict Android single-frame decoder through PR #30, and the bounded single-run stream adapter through PR #32 at `837839417b9b1b3be8b4590c313dc99435936036`. PR #35 merged the Prototype integration-branch CI bootstrap at `aee4d34f6a03c3f427cc546b4b4086748d7c56b4`; its Android, Go and AI checks, and the post-merge integration-branch run, were GREEN. PR #34 then merged the real `AnebClient` raw POST/SSE seam at `3d2c0e497986db750b98d85f93a463697f38a050` with all three remote CI jobs GREEN. Issue #16 remains open and G2 remains active; the next bounded `AnebClient`-backed `PrototypeRawPostTransport` bridge plus one loopback integration test has been dispatched. UI, Room, campaign orchestration and 122-frame topology remain pending. Issue #17's release skeleton remains in P0 HOLD: its capability-reported identity slice is GREEN but uncommitted, while owned-server lifecycle and execution-identity TOCTOU remain HOLD. G3 through G6 remain pending.
...
| G2 — Android vertical slice | `IN_PROGRESS` | PR #29; PR #30; PR #32 merge `837839417b9b1b3be8b4590c313dc99435936036`; PR #35 CI bootstrap merge `aee4d34f6a03c3f427cc546b4b4086748d7c56b4`; PR #34 raw POST/SSE seam merge `3d2c0e497986db750b98d85f93a463697f38a050`; remote Android/Go/AI checks GREEN; issue #16 remains open | Quick + Acceptance campaign and metric gates pass |
| G3 — Fixed Windows package | `PENDING` | issue #17 P0 HOLD; capability-reported identity slice GREEN but uncommitted; owned-server lifecycle and execution-identity TOCTOU remain HOLD | self-contained candidate launches from fresh directory |
...
|---:|---|---|---|---|
| #13 | current PMO conversation | `IN_PROGRESS` | G0/G1 integrated; PR #29/#30/#32/#34/#35 and release P0 work coordinated | G2 exit evidence in progress; G3–G6 pending |
...
| #16 | ANEB_GPT Android | `IN_PROGRESS` | exact projector merged by PR #29; real `done` SSE fixture and strict single-frame decoder merged by PR #30; bounded single-run stream adapter merged by PR #32; CI bootstrap merged by PR #35; real `AnebClient` raw POST/SSE seam merged by PR #34 with all three remote CI jobs GREEN | `AnebClient`-backed `PrototypeRawPostTransport` bridge plus one loopback integration test dispatched; UI, Room, campaign orchestration, 122-frame topology and G2 exit evidence pending |
| #17 | release/packaging | `HOLD_P0` | release skeleton remains HOLD; capability-reported identity slice GREEN but uncommitted; owned-server lifecycle and execution-identity TOCTOU remain HOLD; PR #25 remains HOLD and no release commit is admitted yet | launcher lifecycle/TOCTOU and remaining P0 gates plus #16 app artifact required |
...
boundary, bounded single-run stream adapter and real `AnebClient` raw POST/SSE
seam are merged. The `AnebClient`-backed `PrototypeRawPostTransport` bridge plus
one loopback integration test is dispatched next; UI, Room, campaign
orchestration and 122-frame topology have not yet produced G2 exit evidence. In
parallel, issue #17 has a GREEN but uncommitted capability-reported identity
slice, and must still close owned-server lifecycle and execution-identity TOCTOU
...
1. continue issue #16 from PR #34 merge `3d2c0e497986db750b98d85f93a463697f38a050` with the dispatched `AnebClient`-backed `PrototypeRawPostTransport` bridge plus one loopback integration test before UI, Room, campaign or topology expansion;
2. keep issue #17 and PR #25 in P0 HOLD until the GREEN identity slice is admitted and owned-server lifecycle, execution-identity TOCTOU and all recorded fail-closed defects are GREEN on one revised reviewed head;
