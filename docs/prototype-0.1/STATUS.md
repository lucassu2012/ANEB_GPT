...
| G2 — Android vertical slice | `IN_PROGRESS` | PR #29; PR #30; PR #32 merge `837839417b9b1b3be8b4590c313dc99435936036`; issue #16 remains open | Quick + Acceptance campaign and metric gates pass |
...
|---:|---|---|---|---|
| #13 | current PMO conversation | `IN_PROGRESS` | G0/G1 integrated; PR #29/#30/#32 and release P0 work coordinated | G2 exit evidence in progress; G3–G6 pending |
...
| #16 | ANEB_GPT Android | `IN_PROGRESS` | exact projector merged by PR #29; real `done` SSE fixture and strict single-frame decoder merged by PR #30; bounded single-run stream adapter merged by PR #32 | real `AnebClient` raw POST/SSE seam and adapter wiring next; UI, Room, campaign orchestration, 122-frame topology and G2 exit evidence pending |
| #17 | release/packaging | `HOLD_P0` | capability-reported identity slice GREEN but uncommitted; owned-server lifecycle and execution-identity TOCTOU remain HOLD; PR #25 remains HOLD and no release commit is admitted yet | launcher lifecycle/TOCTOU and remaining P0 gates plus #16 app artifact required |
...
boundary and bounded single-run stream adapter are merged. The real `AnebClient`
raw POST/SSE seam plus adapter wiring is next; UI, Room, campaign orchestration
and 122-frame topology have not yet produced G2 exit evidence. In parallel,
issue #17 has a GREEN but uncommitted capability-reported identity slice, and
must still close owned-server lifecycle and execution-identity TOCTOU before its
release skeleton can leave HOLD or contribute G3/G4 evidence.
...
1. continue issue #16 from PR #32 merge `837839417b9b1b3be8b4590c313dc99435936036` with the real `AnebClient` raw POST/SSE seam plus adapter wiring before UI, Room, campaign or topology expansion;
