...
| S2 三族 M0 收敛 | Token、Realtime、Network 三族均达到 S1；抽取共用采集生命周期，避免三套语义漂移 | 三族能力与旧客户端兼容回归 | catalog/schema/manifest 统一治理 | ［KNOWN｜HIGH］S2-V4f 已冻结 Token preverified publisher CLI 的两参数、退出码和 canonical 机器输出，并保持三族 CLI 兼容。下一门为工具闭包与 PowerShell 薄壳迁移 | EC1、EC2、EC3 全部结案；Token PowerShell 接线、完整三族门禁与 CI 尚待完成 |
