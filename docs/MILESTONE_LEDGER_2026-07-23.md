...
| 轨道 | 最终应实现的内容 | 当前边界 |
|---|---|---|
| P1 手机端 | ANEB App 以真实网络流量模拟 Token 多模态、AI 实时交互和网络综合业务；动态展示关键指标，冻结全量指标、评分、结论与可导出证据 | ［KNOWN｜HIGH］三类引擎、Compose 动态界面、Room v19、strict-v2 结果与导出主链已实现；三族 Quick 正负 READY 已闭环；S3/M1 已完成三族各 5 次、无线权限 granted 的 strict cohort；S3/M2 qualification 的解析、能力、runtime、receipt、单-run LOW/INCONCLUSIVE 本地门与 host campaign controller 已接线，真实 Q1/Q2 各 30 次（每族 10 次）均未执行 |
| P2 测试服务器 | 提供可审计的 echo、token-sim、download、实时交互、综合网络与受控故障原语；任何共享主机变更都可预检、互斥、回滚和验后复核 | ［KNOWN｜HIGH］E-01 仍运行 `aneb-server/0.8.2`，binary SHA-256=`62ff966bf396abe836c6179053ee549110e41e16af569cdeadc97535bc64c96e`，PID=`1295423`，InvocationID=`d975f7c374aa4ef3a490210d0a495e53`；本地源码已固定六个 exact Profile，并完成 `aneb-server/0.8.3` 版本、候选打包和受保护 0.8.2→0.8.3→0.8.2 部署/回滚映射，focused 合同从头复跑 71/71 通过（3 项设计性 skip）；0.8.3 候选二进制尚未构建、提交、通过 CI 或部署，既有 E-01 受锁部署的六项共享指纹不变，owned/stage/watchdog 残留为 0，flock 已释放 |
| P3 标准/业务模型 | 独立模拟 Kimi、DeepSeek、千问等 AI 业务行为，不调用真实 API；以版本化 Profile、模型摘要、校准状态和确定性轨迹供 P1/P2 执行 | ［KNOWN｜HIGH］本地候选为 behavior model 0.3.3、catalog 1.11.0、19 个 Profile、10 个运行包、3 个 execution evidence contracts、D-110 唯一重复性资格政策与资格报告 Schema；分析器核心/CLI、三族 qualification Profile/runtime、Android 本地执行门、server 0.8.3 本地候选/部署合同与 host campaign controller 已接通，但 clean commit/CI、候选构建/部署和真实 Q1/Q2 campaign 尚未完成；没有真实授权数据时不得冒充 calibrated/validated |
| Profile 横切机制 | 每类测试冻结业务类型、全量业务/网络指标、质量目标、动态关键指标、评分算法、结论与网络建议；P1/P2/P3 必须使用同一精确合同 | ［KNOWN｜HIGH］Token Quick 1.2.1、AI 实时 Quick 1.1.1 与 Network Quick 1.2.0 均已完成跨端正负 READY；三族 qualification 1.0.0 已完成 P1/P2/P3 与 host campaign 的本地 exact-policy/profile/runtime 接线，但未形成部署或真实 campaign；三族业务判定器继续独立，不能把 Quick 或本地 GREEN 扩大为正式基线或外场验收完成 |
...
27. ［KNOWN｜HIGH］S3/M2 Batch 3 已完成本地 qualification campaign controller：Q1 Wi-Fi 与 Q2 cellular 各使用冻结的 30-run plan，Q2 必须先验证 Token/Realtime/Network 三份 Q1 PASS 报告及 D-110、Profile、runtime、无线与重复性绑定；CLI 强制显式 stage，删除可由调用方改写次数的 `--repetitions`。runner 在创建任何外部客户端前原子写出 qualification contract、完整 plan 与 Q1 prerequisite manifest/原字节，并且执行阶段只遍历同一 prepared plan；证据目录由已验证 stage 命名为 S3/M2 Q1 Wi-Fi 或 Q2 cellular，避免误归档到旧 S3/M1。完整 `scripts.tests.test_run_repeatability_campaign` 从头复跑 25/25 PASS（18.401s），wall 19.167s、fresh pre/post 均为 0；独立 offline finalizer core+CLI 组合从头 11/11 PASS（26.947s），wrapper wall 28.075s、fresh pre/post=0，可从已清理 campaign 恢复 Token/Realtime/Network 各 10 个唯一 run，在 Room 访问前拒绝 plan/run/Profile/runtime 绑定漂移与重复 terminal run ID；真实 SQLite/strict-v2 Q1 30-run 会按族导出分析，真实 Q2 会把每族结果绑定到自身 Q1 prerequisite，中途家族失败不发布 final 或 partial，并最终一次发布 authority-false manifest。public finalizer CLI 已固定 0/1/2 三态：三族全 PASS→0；处理完成但任一家族 failed→1，并在 canonical stdout 明示状态；usage/证据无效→2 且只写 canonical stderr。随后 campaign25 + cohort24 + cohort CLI9 + exporter4 + finalizer core7 + finalizer CLI4 六模块从头独立交叉回归 73/73 PASS（53.839s），wrapper wall 54.702s、fresh pre/post=0；该轮不与局部门拼接，且只证明 Host 主链交叉兼容。finalizer 又以独立 RED→GREEN 1/1 把 Room 三件套读取故障统一收敛为 `campaign_room_snapshot_invalid`，同时保持 final/partial 不发布与哈希/长度漂移判据不变；修复后 core8 + CLI4 从头 12/12 PASS（19.387s），wrapper wall 20.397s、fresh pre/post=0。随后加入 D-58 7 项与 result-v2 validator 5 项，八模块从头 86/86 PASS（59.896s），wrapper wall 60.842s、fresh pre/post=0，证明 S3/M2 Host 主链与 S3/M1 既有核心语义同轮兼容。之后新增导出后 cohort 文件读取故障 tracer，先精确 RED 命中原始 `OSError` 泄漏，再最小归一化为 `qualification_family_failed:<family>` 且保持 final/partial 零发布；修复后 core9 + CLI4 从头 13/13 PASS（18.472s），wrapper wall 19.656s、fresh pre/post=0；同一八模块随后再次从头复跑 87/87 PASS（64.937s），wrapper wall 66.086s、fresh pre/post=0。随后新增最终 staging→destination rename 故障 tracer，精确 RED 命中原始 `OSError` 泄漏，再归一化为 `finalization_publication_failed` 且清理 partial；修复后 core10 + CLI4 从头 14/14 PASS（21.888s），wrapper wall 23.050s、fresh pre/post=0，同一八模块再从头复跑 88/88 PASS（45.905s），wrapper wall 46.936s、fresh pre/post=0。当前冻结工作树随后单次从头运行完整 `scripts/quality_gate.ps1`，自身 exit 0、wall 31m21.546s、fresh pre/post=0：probe XML 112 files / 628 tests / 0 failure-error-skip，主 Python 922 tests / 16 skip，附加 Python 54 tests，Android lint/assemble、release/spec/result/secret、server/gateway Go 均 PASS；临时 debug candidate 身份为 `com.aneb.probe.codex 0.5.14-codex`、SHA-256=`5ED123F76510DA6F2C068AF8B41514691E2690B9EED4CC09C9F58FBF66823978`。clean commit/CI、0.8.3 候选构建/部署、真实 Q1/Q2 campaign 与正式基线资格仍未完成。

28. ［KNOWN｜HIGH］S3/M2 source/CI 封口：70 个实现文件经 staged diff-check 与仓库标准 secret scan（726 tracked / 70 staged）通过后提交为 `549b2b7aa55a1c6b11f91c98a38083b388461c50` 并推送；GitHub Actions run `30431802801` 七个 job 全部成功，PR #12 保持 Draft、merge state CLEAN，verified Debug artifact 为 `aneb-probe-debug-verified-549b2b7aa55a1c6b11f91c98a38083b388461c50`（artifact `8716100481`）。上一条末尾“clean commit/CI 未完成”是提交前快照，现已被本条取代。该 artifact 仍不等于 actual Product Owner key-pinned Experience Lab candidate，也不等于 Linux server candidate、E-01 部署或 P40 qualification；formal authority 继续为 false。
