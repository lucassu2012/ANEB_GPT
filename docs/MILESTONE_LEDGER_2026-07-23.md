...
| 轨道 | 最终应实现的内容 | 当前边界 |
|---|---|---|
| P1 手机端 | ANEB App 以真实网络流量模拟 Token 多模态、AI 实时交互和网络综合业务；动态展示关键指标，冻结全量指标、评分、结论与可导出证据 | ［KNOWN｜HIGH］三类引擎、Compose 动态界面、Room v19、strict-v2 结果与导出主链已实现；M0-EC1 Token Quick 与 M0-EC2 AI 实时 Quick 的正负 READY 均已完成；网络综合尚未形成同等级 M0 执行合同 |
| P2 测试服务器 | 提供可审计的 echo、token-sim、download、实时交互、综合网络与受控故障原语；任何共享主机变更都可预检、互斥、回滚和验后复核 | ［KNOWN｜HIGH］E-01 运行 `aneb-server/0.8.1`，binary SHA-256=`43e7dc1696f08ec3c460fe094f021274d54492612a910aee0c2db98c39445197`；M0-EC2 验收后 PID/二进制/Docker/eth0/firewall 指纹与进入前一致 |
...
| Profile 横切机制 | 每类测试冻结业务类型、全量业务/网络指标、质量目标、动态关键指标、评分算法、结论与网络建议；P1/P2/P3 必须使用同一精确合同 | ［KNOWN｜HIGH］Token Quick 1.2.1 与 AI 实时 Quick 1.1.1 已分别完成跨端正负 READY；不能扩大表述为所有 Profile 已统一，网络综合仍是下一合同缺口 |
...
6. ［KNOWN｜HIGH］E-01 当前 `aneb-server/0.8.1` 服务端身份已闭合；M0-EC2 最终验收未重复部署服务器，进入前后共享主机指纹一致。
...
6. ［INFERRED｜HIGH］下一最小执行切片为 M0-EC3 网络综合 Quick；复用已验证的 CI provenance、
   正负 READY、同-run 审计与精确清理框架，不重跑 EC1/EC2。
7. ［KNOWN｜HIGH］A6 审核交接当前关闭；不得打开、复制或上传 reviewer HTML、secret、seed、
