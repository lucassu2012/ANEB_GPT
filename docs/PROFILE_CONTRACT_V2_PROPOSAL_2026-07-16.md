...
- 12–20 轮会话，含至少 2 次用户打断和 1 次可控恢复；
...
| LIVE-B12 | 对讲重叠率 | 非计划的双向同时播放时长 / 会话时长 | ≤1% |
...
| LIVE-N06 | 上下行持续有效速率 P5 各≥0.5Mbps |
| LIVE-N07 | loaded RTT P95≤150ms |
| LIVE-N08 | 重连尝试次数、连接迁移事件和无线层协变量完整报告 |
...
### 6.4 Network Comprehensive Score v1（草案）
...
UDP 被网络阻断时应报告“UDP 不可达”，不得伪装成精确 IP 丢包率。是否把 UDP 不可达作为评分封顶项，需 Product Owner 在实施前单独裁定。
