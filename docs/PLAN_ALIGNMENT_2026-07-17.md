...
> S3/M2 提交后更新：clean commit `549b2b7aa55a1c6b11f91c98a38083b388461c50` 已推送，CI run `30431802801` 七个 job 全绿并发布 commit-bound verified Debug artifact；上文“server 候选构建/提交/CI”中的“提交/CI”已完成。剩余顺序是 actual key/pin 与 Android candidate 独立冻结、Linux server 0.8.3 candidate/provenance、受保护 E-01 部署、Q1 30-run/finalization、Q2 30-run/finalization、另行 Product Owner formal-baseline Decision；Debug artifact 不替代其中任一项。
...
［KNOWN｜HIGH］S3/M2 当前里程碑更新：上表“下一门是 clean commit/CI”已由 `549b2b7` / CI `30431802801` 关闭。下一门现为 actual key/pin 与候选身份冻结，然后才是受保护 E-01 部署和 Q1 Wi-Fi；不得把已有 5-run、CI Debug artifact 或候选合同测试提升为现场资格。
