...
| EC3-07 同-run 审计 | HTTP 分类、UDP 序列、原始窗口与 Profile/runtime 交叉绑定 | 离线实现完成：Network 已接入共用设备机械生命周期后端与独立 CLI，冻结独立 audit scope、busy-sentinel schema、启动操作码、Room 合同包名、远端 marker 与 Profile 三文件；独立 consumer 只接受唯一 D82 marker 后日志及空 logcat stderr，并重算客户端、服务端、交叉绑定三份报告。Room 包名去耦后交叉回归 107/107，分段完整门 796+44 及 Android/Go/发布门全部 PASS | 污染、缺号、跨类别身份、旧日志、stderr 或摘要不一致均拒绝 |
