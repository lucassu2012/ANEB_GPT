...
批准记录见 `docs/DECISION_LOG.md` 的 D-36～D-42；完整指标、目标、评分与结论合同见
...
- E-01 已升级到 `aneb-server/0.4.0`，支持 Token SSE、实时交互 WebSocket、HTTP/3 和与 HTTP/3 共用 UDP/8443 的带序号 `ANEB1` 应用探针。
...
5. 完成 Standard/Stress 的长时真机稳定性、取消/切后台/断网恢复和 Android 16 回归，再进入 release 签名候选。
