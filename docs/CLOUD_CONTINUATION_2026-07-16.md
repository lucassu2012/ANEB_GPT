...
- GitHub CI 已从“只监听 main、只编译”升级为 Debug 候选交付流水线：`codex/**` 也会触发，Android 候选等待合同/服务器/网关/行为模型全绿，随后验证 Release 边界、APK 身份与 Debug 签名，生成 APK、机器清单、checksums 与中文安装说明；非 PR 构建追加 GitHub provenance attestation，工件保留 30 天。actionlint、18 项脚本测试、真实本地 APK 打包和全量门禁已通过；本次云端 run 待提交触发后回填。
...
3. M4 的开测前自救、历史批量导出、失败清理和云端 Debug 候选打包已通过本地门禁；下一项先核验真实 GitHub Actions 工件，再做不依赖 ADB 的安装/首次启动/导出整链。签名密钥仍由 Product Owner 在仓库外保管，缺密钥不能冒充正式 Release。
