...
`scripts/scan_repository_secrets.py` 默认只读取 `git ls-files` 返回的已跟踪文件，检测高置信 GitHub、OpenAI、Anthropic、AWS、阿里云、Google、Slack 凭据形状和 PEM 私钥头。
...
- 5 项定向测试覆盖 9 类凭据检测、日志脱敏、合法占位符、二进制跳过与符号链接 fail-closed；Windows 无符号链接权限时该一项明确 skipped，其余 4 项通过。
...
- 完整本地质量门通过：551 项 Android JVM 测试、Lint、Release 边界、候选打包、Schema/catalog、25 项脚本测试、31 项行为模型测试及 Go 服务/网关全部成功。
