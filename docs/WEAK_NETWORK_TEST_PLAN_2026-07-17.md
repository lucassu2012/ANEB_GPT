...
### A. MVP：逐 run 隔离的合成弱网（容量/时延 Profile 已上线）
...
| `weak_recovery@1.0.0` | **待开发**；验证短时中断与恢复 | 候选：基线下行 5Mbps/上行 2Mbps/附加 RTT 80ms；独立注入一次 2s 中断 |
...
容量/时延 Profile 已满足：正常路由独立；同 run 并发只能共享容量、不同 run 独立；seed 可复现；App 启动确认、实时页、结果页和历史均标注“合成弱网”；原始证据冻结目标、回执、排除项与实测指标；取消后不残留系统级损伤；失败样本保持 null/失败事件。`weak_recovery` 仍须单独完成中断隔离、恢复计时和取消回收验收，未上线前不得声称已支持合成断网。
