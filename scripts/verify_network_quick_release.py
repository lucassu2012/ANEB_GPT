...
def _load_json(
    ...
    try:
        return collection_verifier._load_json(
            path, reason, maximum=maximum, require_canonical=True
        )
...
def verify_release(ready_path: str | os.PathLike[str]) -> dict[str, object]:
    ...
    try:
        collection_verifier._assert_directory(ready.parent, "release_root_invalid")
    ...
    report, report_raw = _load_json(
        report_path, maximum=4 * 1024 * 1024, reason="release_report_invalid"
    )
    if (
        collection_verifier._sha256(report_raw)
        != marker["verification_report_sha256"]
    ):
