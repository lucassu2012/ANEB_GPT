...
docker_firewall_snapshot() {
    ...
    iptables-save | grep -E '(^:DOCKER|DOCKER)' || true
    printf '%s\n' '--- ip6tables-save docker rules ---'
    ip6tables-save | grep -E '(^:DOCKER|DOCKER)' || true
}

firewall_fingerprint() {
    firewall_snapshot | hash_stream
}
...
freeze_live_baseline() {
    ...
    BASE_DOCKER_FIREWALL_SHA="$(docker_firewall_fingerprint)"
    BASE_ETH0_QDISC_SHA="$(eth0_qdisc_fingerprint)"
    BASE_FIREWALL_SHA="$(firewall_fingerprint)"
    printf '%s\n' \
...
assert_shared_host_baseline() {
    local label="$1"
    local current_docker current_qdisc current_firewall
    current_docker="$(docker_firewall_fingerprint)"
    current_qdisc="$(eth0_qdisc_fingerprint)"
    current_firewall="$(firewall_fingerprint)"
    if [[ "$current_docker" != "$BASE_DOCKER_FIREWALL_SHA" ]]; then
        echo "SHARED_BASELINE_MISMATCH label=$label surface=docker_iptables" >&2
        return 1
    fi
    ...
    if [[ "$current_firewall" != "$BASE_FIREWALL_SHA" ]]; then
        echo "SHARED_BASELINE_MISMATCH label=$label surface=firewall" >&2
        return 1
    fi
