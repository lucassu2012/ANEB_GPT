...
if generated != [0]:
    raise SystemExit(f"iptables-save Generated header positions invalid: {generated!r}")
if completed != [len(lines) - 1]:
    raise SystemExit(f"iptables-save Completed footer positions invalid: {completed!r}")
...
validate_timestamp(completed_timestamp, "Completed")
...
firewall_snapshot() {
    printf '%s\n' '--- iptables-save ---'
    iptables-save | canonicalize_iptables_save iptables-save || {
        echo 'iptables-save snapshot failed' >&2
        return 1
    }
    printf '%s\n' '--- ip6tables-save ---'
    ip6tables-save | canonicalize_iptables_save ip6tables-save || {
